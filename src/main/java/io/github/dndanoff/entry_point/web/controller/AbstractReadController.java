...
@Validated
@CrossOrigin
public abstract class AbstractReadController <D extends DtoWithId> {
	...
	public ResultListDto<D> findResourcesPaginated(
			@Valid ListInputDto listInput, @Valid @RequestParam(required = false) SearchInputDto searchInput) {
    	log.debug("Received request to findResourcesPaginated() with params listInput={}, searchInput={}", listInput, searchInput);
		if(searchInput != null) {
			return searchResourcesPaginatedInternal(searchInput, listInput);
		}
    ...
    protected abstract ResultListDto<D> searchResourcesPaginatedInternal(SearchInputDto searchInput, ListInputDto listInput) ;
